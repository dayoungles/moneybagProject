...
public class BagDao {
	...
	private RowMapper<Bag> bagMapper = new RowMapper<Bag>() {
		...
		public Bag mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			bag.setCreatedDate(rs.getString("createdDate"));// timestamp종류는 뭘로 가지고 올지? 
	...
	public Bag findBagByUserIdAndBagInfo(int adminId, String bagInfo) {
		...
		try{
			return jdbcTemplate.queryForObject(sql, new Object[]{adminId, bagInfo}, bagMapper);
		}catch(EmptyResultDataAccessException e){
			return null;
		}
	}
	public Bag findBagByBagId(int bagId) {
		...
		try{
			...
			return bag;
//			return jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<Bag>(Bag.class), bagId);
		}catch (EmptyResultDataAccessException e){
			return null;
		}
	}
